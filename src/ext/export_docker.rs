...
fn render_base_nix(lock: &Path, shell_file: &Path, name: &str, include: &[PathBuf]) -> String {
	let includes: String = include
		...
		.collect();
	BASE_NIX_TEMPLATE
		.replace("@LOCK@", &nix_string(lock))
		.replace("@SHELL@", &nix_string(shell_file))
		.replace("@NAME@", name)
...
const BASE_NIX_TEMPLATE: &str = r##"# docker-base.nix — a reproducible OCI image of the composed clinix env.
...
let
  ...
  fetch = node:
    ...
    else throw "docker-base.nix: unsupported input type '${i.type}'";
...
/// A path as an escaped, double-quoted nix string literal (coerced to a path by
/// `readFile`/`import`). Escapes `\`, `"`, and `${`.

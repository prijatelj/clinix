...
mod tests {
    ...
    // Real locks from this repo's fixture corpus (byte-for-byte). SIMPLE is a
    // single github input; FOLLOWS exercises the string-vs-array `inputs`
    // polymorphism and multiple github nodes.
    const SIMPLE: &str = include_str!("../../tests/fixtures/simple.lock");
    const FOLLOWS: &str = include_str!("../../tests/fixtures/follows.lock");
    // Live `nix flake lock` output from `env init` (has `lastModified`), so the
    // round-trip is checked against what clinix's own scaffolding produces.
    const GENERATED: &str = include_str!("../../tests/fixtures/generated.lock");
